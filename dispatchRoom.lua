...
imgui.OnInitialize(function()
	gta_sa_map = imgui.CreateTextureFromFile(
		getWorkingDirectory().."\\resource\\San_Andreas_aerial_view.jpg"
	)
end)
...
function main()
	...
	while true do
		...
		if status and not is_dispatcher and is_equal(info_to_send, {}) then
			...
			info_to_send.x, info_to_send.y = math.floor(x), math.floor(y)
			info_to_send.mark = marking

			if isCharInAnyCar(PLAYER_PED) or isCharOnAnyBike(PLAYER_PED) then
				local veh = storeCarCharIsInNoSave(PLAYER_PED)
				info_to_send.veh = getCarModel(veh)
			else
				info_to_send.veh = 0
			end
...
function sampev.onServerMessage(color, message)
	if group_chat_msg_template then
		if message:find(group_chat_msg_template) then
			-- Ник отправившего сообщение
			local nick, json = message:match(group_chat_msg_template)
...
function show_information_about911(text, number, caller_nickname, x, y)
	local caller_id = sampGetPlayerIdByNickname(caller_nickname)
	local district = get_district_by_coords(x, y)

	sampAddChatMessage("=========== 911 ==========", -1)
	sampAddChatMessage("Район: "..district, -1)
	sampAddChatMessage("Ситуация #"..tostring(number)..": "..text, -1)
	sampAddChatMessage("Звонивший: "..caller_nickname.."["..caller_id.."]", -1)
...
function send_table_in_group_chat(t)
	local cmd = ini.chat_message.cmd
	local cmd_end = ini.chat_message.cmd_end or ""
	-- если последний символ команды не пробел
	if cmd[#cmd] ~= " " then
		-- то добавляем его в конец
		cmd = cmd.." "
	end

	sampSendChat(cmd..encodeJson(t)..cmd_end)
end
...
function dialog_input(input_text)
	...
	local _, button, _, input = sampHasDialogRespond(dialog_id)
	return button == 1 and input ~= "" and input
end

function question_dialog(question)
	-- Выводит диалог с клавишами "Да"/"Нет" с текстом question в кодировке Windows-1251.
	sampShowDialog(
		dialog_id,
		thisScript().name,
		u8:decode(question),
	...
	while sampIsDialogActive(dialog_id) do wait(100) end
	local _, button, _, _ = sampHasDialogRespond(dialog_id)
	return button == 1
end


function set_race_checkpoint(x, y, z)
	if not z then
		z = 10
	end
    ...
    checkpoint = createCheckpoint(2, x, y, z, nil, nil, nil, 5)
    changeBlipColour(checkpoint, 0xFF0000)

    lua_thread.create(function()
	    repeat
	        wait(0)
	        local xplayer, yplayer, zplayer = getCharCoordinates(PLAYER_PED)
	    until getDistanceBetweenCoords3d(x, y, z, xplayer, yplayer, zplayer) < radius or not doesBlipExist(checkpoint)
...
function get_district_by_coords(x, y)
...
function is_equal(t1, t2)
	for i, it in pairs(t1) do
		if t2[i] ~= it then
			return false
		end
	end

	for i, it in pairs(t2) do
		if t1[i] ~= it then
			return false
		end
	end
